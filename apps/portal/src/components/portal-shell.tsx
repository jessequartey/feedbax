...
export function ConnectorOutage({ onRetry, activePage = 'feedback', wide = false }: { onRetry: () => void; activePage?: Page; wide?: boolean }) {
  const subject = activePage === 'roadmap' ? 'The roadmap' : activePage === 'changelog' ? 'The changelog' : 'Feedback'
  return <PortalShell branding={portalBrandingFallback} activePage={activePage} wide={wide}><section className="state-page outage" role="alert"><span className="outage-mark" aria-hidden="true">!</span><p className="eyebrow">Connection interrupted</p><h1>{subject} is temporarily unavailable</h1><p>We couldn’t reach the connected workspace. Your account and existing feedback are safe.</p><div className="state-actions"><button className="primary-button" type="button" onClick={onRetry}>Try again</button><a href={portalBrandingFallback.supportUrl}>Contact support</a></div></section></PortalShell>
}
