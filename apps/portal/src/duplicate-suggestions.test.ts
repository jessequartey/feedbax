...
import { PublicFeedbackItemSchema, type PublicFeedbackItem } from '@feedbax/core'
import {
...
const item = (id: string, title: string, voteCount = 0): PublicFeedbackItem =>
  PublicFeedbackItemSchema.parse({
    id, title, description: `${title} description`, type: 'feature',
    author: { id: 'author', displayName: 'Author' }, status: null, category: null,
    tags: [], voteCount, commentCount: 0,
...
describe('duplicate suggestion matching', () => {
  it('normalizes case, accents, punctuation, and whitespace', () => {
    expect(normalizeSuggestionTitle('  Éxport—TO   CSV! ')).toBe('export to csv')
  })

  it('matches exact, reordered, and mildly mistyped titles', () => {
    expect(suggestionScore('Export CSV', 'export csv')).toBe(1)
    expect(suggestionScore('CSV export', 'Export CSV')).toBeGreaterThan(.75)
    expect(suggestionScore('Export to CSF', 'Export to CSV')).toBeGreaterThan(.8)
  })

  it('excludes unrelated titles', () => {
    expect(rankDuplicateSuggestions('Export to CSV', [item('one', 'Dark mode')])).toEqual([])
  })
  ...
  it('orders by score, breaks ties by votes, and returns at most three', () => {
    ...
    expect(ranked.map(({ id }) => id)).toEqual(['exact-high', 'exact-low', 'fourth'])
  })
})
