import type { PublicChangelogDetail } from '@feedbax/core'
...
export function changelogMetadata(detail: PublicChangelogDetail) {
  const { entry } = detail
  const title = `${entry.title} · ${portalBranding.productName}`
  ...
  const image = entry.coverImageUrl ?? portalBranding.socialPreviewImage
  return {
    meta: [
      { title },
...
function ChangelogDetailPage() {
  ...
  return (
    <PortalShell activePage="changelog">
      <a className="back-link" href="/changelog">
        ← All updates
      </a>
      <article className="changelog-detail">
        ...
        {entry.coverImageUrl ? (
          <img
            className="changelog-detail-cover"
        ...
        <div
          className="markdown changelog-detail-content"
          dangerouslySetInnerHTML={{
            __html: renderSanitizedMarkdown(entry.description),
          }}
        />
        {relatedFeedback.length ? (
          <section
            className="shipped-feedback"
            aria-labelledby="shipped-feedback-heading"
          >
            ...
            <div>
              {relatedFeedback.map((item) => (
                <a
                  ...
                  key={item.id}
                  aria-label={`${item.status!.name}: ${item.title}`}
                >
    ...
    <PortalShell activePage="changelog">
      <section className="state-page">
        ...
        <div className="state-actions">
          <a className="primary-button" href="/changelog">
            Browse updates
          </a>
        </div>
      </section>
