...
export function metadataFor(detail: PublicFeedbackDetail) {
  const title = `${detail.item.title} · ${portalBranding.productName}`
  const description = detail.item.description
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160)
  ...
  return {
    meta: [
      ...
      ...(portalBranding.socialPreviewImage
        ? [{ property: 'og:image', content: portalBranding.socialPreviewImage }]
        : []),
      ...
      ...(portalBranding.socialPreviewImage
        ? [
            {
              name: 'twitter:image',
              content: portalBranding.socialPreviewImage,
            },
          ]
        : []),
    ],
...
async function mutate(path: string, input: unknown): Promise<unknown> {
  const response = await fetch(path, {
    ...
    headers: {
      ...
      'x-feedbax-return-path': location.pathname,
    },
  ...
  if (!response.ok) {
    throw new ApplicationApiError(
      body.error?.message ?? 'This action is temporarily unavailable.',
      response.status,
      body.error?.code ?? 'UNEXPECTED_ERROR',
      body.error?.requestId,
      body.error?.retryable ?? response.status >= 500,
      body.error?.retryAfterSeconds,
      body.error?.loginLocation,
    )
  }
...
function FeedbackDetailPage() {
  ...
  const [subscribed, setSubscribed] = useState(
    detail.subscription.isSubscribed ?? false,
  )
  const [pending, setPending] = useState<
    'vote' | 'comment' | 'subscribe' | null
  >(null)
  ...
  const [comments, setComments] = useState<CommentRow[]>(() =>
    detail.comments.items.map((comment) => ({ comment })),
  )
  useEffect(() => {
    void fetch('/api/vote-state', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ feedbackItemIds: [item.id] }),
    })
      .then(async (response) => (response.ok ? response.json() : null))
      .then((value) => {
        if (value)
          setVoted(
            VoteStateResponseSchema.parse(value).items[0]?.voted ?? false,
          )
      })
      .catch(() => undefined)
  }, [item.id])
  ...
  const vote = async () => {
    ...
    setVoted(next)
    setVotes(Math.max(0, votes + (next ? 1 : -1)))
    try {
      const result = SetVoteResultSchema.parse(
  ...
  const subscribe = async () => {
    ...
    setActionError(undefined)
    try {
      await mutate('/api/subscribe', {
        target: { type: 'feedback', id: item.id },
        subscribed: next,
      })
      setSubscribed(next)
      setMessage(
        next ? 'You’re subscribed to updates.' : 'Subscription removed.',
      )
    } catch (error) {
      setActionError(error as Error)
      setMessage((error as Error).message)
  ...
  const persistComment = async (
    ...
    try {
      const saved = PublicCommentSchema.parse(
      ...
      setComments((current) =>
        current.map((row) =>
          row.comment.id === temporaryId ? { comment: saved } : row,
        ),
      )
      ...
      setComments((current) =>
        current.map((row) =>
          row.comment.id === temporaryId
            ? { ...row, state: 'failed', error: (error as Error).message }
            : row,
        ),
      )
  ...
  const comment = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!parsed.success) {
      setMessage(parsed.error.issues[0]?.message ?? 'The comment is invalid.')
      return
    }
    const now = new Date().toISOString()
    const temporaryId = `pending-${clientRequestId}`
    const temporary = PublicCommentSchema.parse({
      id: temporaryId,
      feedbackItemId: item.id,
      body,
    ...
    setComments((current) => [
      ...current,
      { comment: temporary, clientRequestId, state: 'pending' },
    ])
    form.reset()
      ...
      <div className="detail-layout">
        <article className="feedback-detail">
          <header className="detail-heading">
            ...
            <button
              className="vote detail-vote"
              type="button"
              aria-pressed={voted}
              disabled={pending === 'vote'}
              onClick={() => void vote()}
              aria-label={`${voted ? 'Remove vote from' : 'Vote for'} ${item.title}; ${votes} votes`}
            >
          ...
          <div
            className="markdown detail-description"
            dangerouslySetInnerHTML={{
              __html: renderSanitizedMarkdown(item.description),
            }}
          />
          <div className="detail-byline">
            {item.author.avatarUrl ? (
              <img src={item.author.avatarUrl} alt="" width="38" height="38" />
            ) : (
            ...
            <p>
              Shared by <strong>{item.author.displayName}</strong>
              <small>
                Created{' '}
          ...
          <section className="detail-section" aria-labelledby="comments-title">
                    ...
                    >
                      <div>
                        <span>
                          ...
                          {comment.authorKind !== 'customer' ? (
                            <small className="author-badge">
                              {comment.authorKind === 'administrator'
                                ? 'Administrator'
                                : 'Team'}
                            </small>
                          ) : null}
                      ...
                      <div
                        className="markdown"
                        dangerouslySetInnerHTML={{
                          __html: renderSanitizedMarkdown(comment.body),
                        }}
                      />
                      {row.state === 'failed' ? (
                        <div className="comment-retry" role="alert">
                          <span>{row.error ?? 'Comment failed.'}</span>
                          <button
                            type="button"
                            onClick={() =>
                              void persistComment(
                          ...
                          <button
                            ...
                            onClick={() =>
                              setComments((current) =>
                                current.filter(
                                  (candidate) =>
                                    candidate.comment.id !== comment.id,
                                ),
                              )
                            }
                          >
            ...
            >
              ...
              <small id="comment-help">
                Markdown is supported. For privacy, don’t include email
                addresses.
              </small>
        ...
        <aside className="detail-context" aria-label="Feedback details">
          <dl>
            <div>
              <dt>Status</dt>
              <dd>{item.status?.name ?? 'No status'}</dd>
            </div>
            <div>
              <dt>Type</dt>
              <dd>{item.type}</dd>
          ...
          {detail.roadmap.length || detail.releases.length ? (
            <section className="related">
              <h2>Related activity</h2>
              {detail.roadmap.map((entry) => (
                <a href="/roadmap" key={entry.id}>
                  <small>
                    Roadmap{entry.status ? ` · ${entry.status.name}` : ''}
                  </small>
              ...
              {detail.releases.map((release) => (
                <a
                  href={`/changelog/${encodeURIComponent(release.slug)}`}
                  key={release.changelogEntryId}
                >
                  <small>
                    {item.status?.name ?? 'Released'}
          ...
          {actionError ? (
            <ErrorState error={actionError} scope="action" />
          ) : null}
        </aside>
      </div>
    </PortalShell>
...
function FeedbackNotFound() {
  return (
    <PortalShell activePage="feedback">
      <ApplicationState
        kind="not-found"
        title="This feedback isn’t available"
        actions={
