...
function reader(
  overrides: Partial<PublicConnectorReader> = {},
): PublicConnectorReader {
  const release = ChangelogPageSchema.parse({
    items: [
      {
        id: 'change-1',
        slug: 'shipped',
...
describe('feedback detail aggregate', () => {
  ...
  it('does not perform secondary reads for missing or private items', async () => {
    ...
    const detail = await loadFeedbackDetail(
      reader({
  ...
  it('preserves release order while serializing only compact references', async () => {
    const releases = ChangelogPageSchema.parse({
      items: [
        {
          id: 'change-2',
          slug: 'second-release',
          title: 'Second release',
          description: 'Private body two',
          publishedAt: '2026-07-13T08:00:00Z',
          tags: [],
          linkedFeedbackItemIds: ['feedback-1', 'internal-id'],
          createdAt: item.createdAt,
          updatedAt: item.updatedAt,
        },
        {
    ...
    expect(detail?.releases.map(({ slug }) => slug)).toEqual([
      'second-release',
      'first-release',
    ])
