...
} from './public-feedback.server.js'

describe('public feedback list', () => {
  it('maps public URL filters to domain filters', () => {
    const result = parseFeedbackRequest(
      new URL(
        'https://feedbax.test/api/feedback?q=export&status=open&status=planned&category=feature&sort=updated&cursor=next&limit=12',
      ),
    )
    expect(result).toEqual({
      filter: {
        search: 'export',
        statusIds: ['open', 'planned'],
        categoryId: 'feature',
  ...
  it.each(['sort=oldest', 'status=private', 'category=unknown', 'limit=101'])(
    'rejects invalid public query %s',
    (query) => {
      expect(() =>
        parseFeedbackRequest(
          new URL(`https://feedbax.test/api/feedback?${query}`),
        ),
      ).toThrow()
    },
  )

  it('distinguishes missing connector credentials from empty feedback', async () => {
  ...
  it('sanitizes invalid-query errors', async () => {
    const response = await feedbackListResponse(
      new Request('https://feedbax.test/api/feedback?category=secret'),
    )
...
describe('feedback suggestions', () => {
  ...
  it('validates suggestion titles', () => {
    expect(
      parseSuggestionRequest(
        new URL('https://feedbax.test/api/feedback/suggestions?title=export'),
      ),
    ).toBe('export')
    expect(() =>
      parseSuggestionRequest(
        new URL('https://feedbax.test/api/feedback/suggestions?title=ab'),
      ),
    ).toThrow()
    expect(() =>
      parseSuggestionRequest(
        new URL(
          `https://feedbax.test/api/feedback/suggestions?title=${'x'.repeat(201)}`,
        ),
      ),
    ).toThrow()
  })
  ...
  it('uses one bounded connector read and returns schema-valid matches', async () => {
    ...
    const response = await feedbackSuggestionsResponse(
      new Request(
        'https://feedbax.test/api/feedback/suggestions?title=export%20csv',
      ),
      {
        listFeedback: async (filter, page) => {
          received = { filter, page }
          return {
            value: { items: [candidate], hasMore: false },
            cacheStatus: 'hit',
          }
        },
      },
  ...
  it('reports unavailable suggestions without connector credentials', async () => {
    const response = await feedbackSuggestionsResponse(
  ...
  it('sanitizes connector failures', async () => {
    const response = await feedbackSuggestionsResponse(
      ...
      {
        listFeedback: async () => {
          throw new Error('secret connector detail')
        },
      },
    )
    expect(response.status).toBe(503)
