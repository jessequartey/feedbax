...
assert.ok(etag)
assert.equal((await request('/api/cache', { headers: { 'if-none-match': etag } })).status, 304)
const mutation = await request('/api/mutation', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'increment' }) })
assert.equal(mutation.status, 200)
const setCookie = mutation.headers.get('set-cookie') || ''
for (const flag of ['HttpOnly', 'Secure', 'SameSite=Lax', 'Path=/', 'Max-Age=3600']) assert.match(setCookie, new RegExp(flag, 'i'))
assert.equal((await request('/api/mutation', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })).status, 422)
assert.equal((await request('/api/error?kind=expected')).status, 400)
const unexpected = await request('/api/error')
assert.equal(unexpected.status, 500)
assert.deepEqual(await unexpected.json(), { error: { code: 'INTERNAL_ERROR', message: 'An unexpected error occurred.' } })
const notion = await request('/api/notion')
assert.equal(notion.status, 200, 'Notion check requires valid NOTION_TOKEN and NOTION_RESOURCE_ID')
