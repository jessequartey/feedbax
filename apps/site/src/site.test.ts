...
import { docsNavigation, matchesSearch, searchIndex, statusLabels } from './lib/site'
...
describe('site content model', () => {
  it('publishes every required documentation page in a stable order', () => {
    expect(docsNavigation.map((item) => item.slug)).toEqual([
      '', 'product-vision', 'quickstart', 'configuration', 'authentication-handoff',
      'notion-setup', 'deployment', 'roadmap', 'contributing',
    ])
  })

  it('uses only the agreed public status labels', () => {
    expect(statusLabels).toEqual(['Available', 'Foundation only', 'Planned for v0.0.2', 'Deferred'])
  })

  it('builds a static search record for every documentation page', () => {
    expect(searchIndex).toHaveLength(docsNavigation.length)
    expect(searchIndex.find((item) => matchesSearch(item.keywords, 'identity handoff'))?.href).toBe('/docs/authentication-handoff')
    expect(searchIndex.every((item) => item.href.startsWith('/docs'))).toBe(true)
  })
