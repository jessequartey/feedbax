...
async function withCommentLock<T>(
  key: string,
  work: () => Promise<T>,
): Promise<T> {
...
export function createNotionMutationService(options: NotionMutationOptions) {
  ...
  const voterKey = async (userId: string) => {
    if (
      !options.interactionHashKey ||
      new TextEncoder().encode(options.interactionHashKey).byteLength < 32
    )
      throw new Error('Voting is not configured.')
    const key = await crypto.subtle.importKey(
      'raw',
      new TextEncoder().encode(options.interactionHashKey),
  ...
  return {
    async submit(
      ...
      const properties: Record<string, unknown> = {
        ...
        [fields.description.property]: {
          rich_text: richText(input.description),
        },
        [fields.feedbackType.property]: {
          select: {
            name: options.setup.feedbackTypes?.[input.type] ?? input.type,
          },
        },
      ...
      if (category && input.categoryId)
        properties[category.property] = {
          select: {
            name:
              options.setup.categories?.[input.categoryId] ?? input.categoryId,
          },
        }
      ...
      try {
        response = await fetcher(`${API}/pages`, {
          ...
          body: JSON.stringify({
            parent: {
              type: 'data_source_id',
              data_source_id: options.setup.dataSourceId,
            },
            properties,
          }),
        })
      ...
      const item = PublicFeedbackItemSchema.parse({
        ...
        status: {
          ...
          isTerminal:
            options.setup.statusDefinitions?.open?.isTerminal ?? false,
        },
        category: input.categoryId
          ? {
              id: input.categoryId,
              name:
                options.setup.categories?.[input.categoryId] ??
                input.categoryId,
              order: 0,
            }
          : null,
    ...
    async setVote(userId: string, input: SetVoteInput): Promise<SetVoteResult> {
      ...
      return withVoteLock(input.feedbackItemId, async () => {
        ...
        const feedback = await notion<{
          parent?: { type?: string; data_source_id?: string }
          properties?: Record<string, { checkbox?: boolean }>
        }>(`/pages/${encodeURIComponent(input.feedbackItemId)}`, {
          method: 'GET',
        })
        const visibility = options.setup.fields.optional?.visibility
        if (
          feedback.parent?.type !== 'data_source_id' ||
          feedback.parent.data_source_id !== options.setup.dataSourceId ||
          (visibility &&
            feedback.properties?.[visibility.property]?.checkbox === false)
        )
          throw new Error('Feedback was not found.')
        const filter = {
          and: [
            {
              property: setup.fields.feedbackItem.property,
              relation: { contains: input.feedbackItemId },
            },
            {
              property: setup.fields.voterKey.property,
        ...
        }>(`/data_sources/${encodeURIComponent(setup.dataSourceId)}/query`, {
          method: 'POST',
          body: JSON.stringify({ page_size: 100, filter }),
        })
        const existing = found.results?.[0]
        const active =
          existing?.properties?.[setup.fields.active.property]?.checkbox ===
          true
        if (active !== input.voted) {
          if (existing)
            await notion(`/pages/${encodeURIComponent(existing.id)}`, {
              method: 'PATCH',
              body: JSON.stringify({
                properties: {
                  [setup.fields.active.property]: { checkbox: input.voted },
                },
              }),
            })
          else if (input.voted)
            await notion('/pages', {
              ...
              body: JSON.stringify({
                parent: {
                  type: 'data_source_id',
                  data_source_id: setup.dataSourceId,
                },
                properties: {
                  [setup.fields.key.property]: {
                    title: richText(`${input.feedbackItemId}:${hash}`),
                  },
                  [setup.fields.feedbackItem.property]: {
                    relation: [{ id: input.feedbackItemId }],
                  },
                  [setup.fields.voterKey.property]: {
                    rich_text: richText(hash),
                  },
        ...
        const activeVotes = await queryAll<{
          properties?: Record<
            string,
          ...
          filter: {
            and: [
              ...
              {
                property: setup.fields.active.property,
        ...
        const unique = new Set(
          activeVotes
            .map((row) =>
              row.properties?.[setup.fields.voterKey.property]?.rich_text
        ...
        await notion(`/pages/${encodeURIComponent(input.feedbackItemId)}`, {
          method: 'PATCH',
          body: JSON.stringify({
            properties: { [countField.property]: { number: voteCount } },
          }),
        })
        if (options.cache)
    ...
    async createComment(
      author: PublicUser,
      authorKind: 'customer' | 'team' | 'administrator',
      ...
      return withCommentLock(input.feedbackItemId, async () => {
        ...
        const existing = await notion<{ results?: CommentPage[] }>(
          `/data_sources/${encodeURIComponent(setup.dataSourceId)}/query`,
          {
            method: 'POST',
            body: JSON.stringify({
              ...
              filter: {
                and: [
                  {
                    property: setup.fields.key.property,
                    title: { equals: input.clientRequestId },
                  },
                  {
                    property: setup.fields.feedbackItem.property,
                    relation: { contains: input.feedbackItemId },
                  ...
                  {
                    property: setup.fields.authorId.property,
                    rich_text: { equals: author.id },
                  },
                ],
              },
            }),
        ...
        if (!page)
          page = await notion<CommentPage>('/pages', {
            method: 'POST',
            body: JSON.stringify({
              parent: {
                type: 'data_source_id',
              ...
              properties: {
                [setup.fields.key.property]: {
                  title: richText(input.clientRequestId),
                },
                [setup.fields.feedbackItem.property]: {
                  relation: [{ id: input.feedbackItemId }],
                },
                [setup.fields.body.property]: {
                  rich_text: richText(input.body),
                },
                [setup.fields.authorId.property]: {
                  rich_text: richText(author.id),
                },
                [setup.fields.authorName.property]: {
                  rich_text: richText(author.displayName),
                },
                ...(setup.fields.authorAvatar && author.avatarUrl
                  ? {
                      [setup.fields.authorAvatar.property]: {
                        url: author.avatarUrl,
                      },
                    }
                  : {}),
                [setup.fields.authorKind.property]: {
                  select: { name: authorKind },
                },
              },
            }),
          })
        const all = await queryAll<unknown>(setup.dataSourceId, {
          page_size: 100,
          filter: {
            property: setup.fields.feedbackItem.property,
            relation: { contains: input.feedbackItemId },
        ...
        await notion(`/pages/${encodeURIComponent(input.feedbackItemId)}`, {
          ...
          body: JSON.stringify({
            properties: {
              [options.setup.fields.commentCount.property]: {
                number: all.length,
              },
            },
          }),
        ...
        return CreateCommentResultSchema.parse({
          id: page.id,
          feedbackItemId: input.feedbackItemId,
          body: input.body,
          author,
          ...
          updatedAt: page.last_edited_time ?? page.created_time ?? now,
        })
      })
    },
    async voteStates(userId: string, feedbackItemIds: readonly string[]) {
      const setup = options.setup.votes
      if (!setup || !options.interactionHashKey)
        throw new Error('Voting is not configured.')
      const hash = await voterKey(userId)
      ...
      const active = new Set(
        (result.results ?? []).flatMap(
          (row) =>
            row.properties?.[setup.fields.feedbackItem.property]?.relation?.map(
              ({ id }) => id,
            ) ?? [],
        ),
      )
      return feedbackItemIds.map((feedbackItemId) => ({
        feedbackItemId,
        voted: active.has(feedbackItemId),
      }))
    },
  }
}
